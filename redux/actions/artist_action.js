...
export function PostDataArtistAction(artistData) {
  return function(dispatch) {
    ...
    API.post('/artist', artistData)
      .then(response =>
        dispatch({type: POST_ARTIST_SUCCSESS, payload: response.data.message}),
      )
      .catch(error =>
        dispatch({
...
export function getDataArtistsAction() {
  return function(dispatch) {
    dispatch({
    ...
    API.get('/artists')
      .then(response =>
        dispatch({type: GET_ARTIST_SUCCSESS, payload: response.data.data}),
      )
      .catch(response =>
        dispatch({
...
export function getArtistBySongAction(name) {
  return function(dispatch) {
    ...
    API.get('/song/artist/' + name)
      .then(response =>
        dispatch({
          type: GET_ARTIST_BY_SONG_SUCCESS,
